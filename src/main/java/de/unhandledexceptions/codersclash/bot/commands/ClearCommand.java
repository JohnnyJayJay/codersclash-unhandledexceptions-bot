    ...
    public void onCommand(CommandEvent event, Member member, TextChannel channel, String[] args) {
        ...
        if (Permissions.getPermissionLevel(member) >= 3) {
            if (args.length == 1 && args[0].matches("(\\d{1,2}|100)")) {
                ...
                event.getMessage().delete().queue((v) -> {
                    ...
                    if (amount == 1) {
                        messages.get(0).delete().queue(
                                ...
                                (throwable) -> sendMessage(channel, Type.WARNING, String.format("Something went wrong:\n```\n%s```", throwable.getMessage())).queue());
                    } else {
                ...
                sendMessage(channel, Type.WARNING, String.format("Correct usage: `%sclear <amount>` (amount is a number between 1 and 100",
                        settings.getPrefix(event.getGuild().getIdLong()))).queue();
            }
        } else {
            sendMessage(channel, Type.ERROR, "You do not have permission to execute this command. " + member.getAsMention()).queue();
        }
    }

    @Override
    public String info(Guild guild) {
        return String.format("Description: clears up to 100 messages at a time.\nUsage: `%sclear <amount>`\nPermission level: `3`", settings.getPrefix(guild.getIdLong()));
    }
}
