...
public class Management extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        if (Bot.getBotOwners().contains(event.getAuthor().getIdLong())) {
            ...
            String regex = prefix + "manage (((status)|(restart)|(shutdown)) ((complete)|(\\d)))";
            ...
            } else if (event.getMessage().getContentRaw().matches(prefix + "manage commandsettings ((activate)|(deactivate))")) {
                if (args[1].equals("deactivate") && bot.getCommandSettings().isActivated()) {
                    bot.getCommandSettings().deactivate();
                    event.getChannel().sendMessage("CommandSettings wurden deaktiviert.").queue();
                } else if (args[1].equals("activate") && !bot.getCommandSettings().isActivated()) {
